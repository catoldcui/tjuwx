  ...
  <head>
    ...
    <s:iterator value="#session.searchLetterlist" id="letter" status="st">
        <tr
             ...
             >
        	<td><s:property value="%{#st.count + ( #session.searchPageNum - 1 ) * @org.tju.wx.action.LetterAction@ELENUM}"/></td>
    ...
    <div align="right">
        ...
        <s:else>
            <a class="btn btn-primary btn-lg active"  href="searchletter.action?searchPageNum=1&key=<s:property value='#session.key'/>">首页</a>
            &nbsp;&nbsp;&nbsp;
            <a class="btn btn-primary btn-lg active"  href="searchletter.action?searchPageNum=<s:property value='#session.searchPageNum - 1'/>&key=<s:property value='#session.key'/>">上一页</a>
        </s:else>
        
        <s:if test="#session.searchPageNum != #session.searchTotalpage">
            <a class="btn btn-primary btn-lg active"  href="searchletter.action?searchPageNum=<s:property value='#session.searchPageNum + 1'/>&key=<s:property value='#session.key'/>">下一页</a>
            &nbsp;&nbsp;&nbsp;
            <a class="btn btn-primary btn-lg active"  href="searchletter.action?searchPageNum=<s:property value='#session.searchTotalpage'/>&key=<s:property value='#session.key'/>">尾页</a>
        </s:if>
        ...
        <font size="3px">共<font color="red"><s:property value="#session.searchTotalpage"/></font>页 </font>
        <font size="3px">当前第<font color="red"><s:property value="#session.searchPageNum"/></font>页</font>
        <font size="3px">共<font color="red"><s:property value="#session.searchCount"/></font>条记录</font>
        <br/>

        <form action="searchletter" onSubmit="return validate();">
            <font size="4">跳转至
            <input type="text" name="searchPageNum" size="2"/>页
            <input name="key" size="2" type="hidden" vaule="<s:property value='#session.key'/>"/>
